...
def test_fetch_data_availability_data_current_and_archive():
    region_data = fetch_data.get_duid_availability_data(
        ...
        'dummy_directory')
    expected_data = pd.DataFrame(
        columns=['SETTLEMENTDATE', 'DUID', 'AVAILABILITY', 'TOTALCLEARED', 'INITIALMW', 'RAMPDOWNRATE',
                 'RAMPUPRATE'],
        data=[('2020/01/01 01:00:00', 'A', 101.0, 81.0, 71.0, 122.0, 126.0),
    ...
    expected_data['SETTLEMENTDATE'] = pd.to_datetime(expected_data['SETTLEMENTDATE'])
    region_data = region_data.sort_values(['SETTLEMENTDATE', 'DUID']).reset_index(drop=True)
    expected_data = expected_data.sort_values(['SETTLEMENTDATE', 'DUID']).reset_index(drop=True)
    pd.testing.assert_frame_equal(region_data, expected_data)
